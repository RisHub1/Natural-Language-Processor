...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    if (! node) return 0; 
    return 1 + std::max(size_impl(node->left), size_impl(node->right));
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    if (! node) return true; 
    Node * maxNode = max_element_impl(node->left);
    Node * minNode = min_element_impl(node->right);
    if (minNode && ! less(node->datum, minNode->datum)) return false; 
    if (maxNode && ! less(maxNode->datum, node->datum)) return false; 
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    {
      Node * checkLeft = min_greater_than_impl(node->left, val, less);
